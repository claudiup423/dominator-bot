...
def build_obs(packet: GamePacket, index: int, team: int, field_info: FieldInfo) -> np.ndarray:
    ...
    obs.append(1.0 if my_car.air_state == 0 else 0.0) 
    obs.append(1.0 if my_car.air_state == 0 else 0.0)  # has_flip approx
    ...
    for i, player in enumerate(packet.players):
        ...
        p_data.append(1.0 if player.air_state == 0 else 0.0)
        p_data.append(1.0 if player.air_state == 0 else 0.0)
...
if __name__ == "__main__":
    bot = PPOBot("bossdeboss/dominance-ppo")
    bot.run()
